...
import ParticipantsPanel, {
  Participant,
} from "@/components/calling/ParticipantsPanel";
...
const CallingPage = () => {
    ...
    const fetchHostCap = async () => {
      try {
        ...
        if (!PACKAGE_ID || !REGISTRY_OBJECT_ID) {
          setChainStatus(
            "Missing PACKAGE_ID or REGISTRY_OBJECT_ID. Configure env vars."
          );
          return;
        }
        ...
        for (const o of caps.data) {
          ...
          if (
            typeof capRoomId === "string" &&
            capRoomId.toLowerCase() === roomId.toLowerCase()
          ) {
            setHostCapId(id);
        ...
        if (target === "offer") {
          ...
          if (off?.sdp) {
            ...
            if (pc.signalingState !== "stable") {
              console.log(
                "[Signaling] GUEST: Ignoring offer, wrong state:",
                pc.signalingState
              );
              return;
            }
          ...
          if (ans?.sdp) {
            ...
            if (pc.signalingState !== "have-local-offer") {
              console.log(
                "[Signaling] HOST: Ignoring answer, wrong state:",
                pc.signalingState
              );
              return;
            }
            ...
            console.log(
              "[Signaling] HOST: Received answer from server (state:",
              pc.signalingState + ")"
            );
            ...
            console.log(
              "[Signaling] HOST: Set remote description, new state:",
              pc.signalingState
            );
  ...
  const toggleAudio = () => {
    ...
    setParticipants((prev) =>
      prev.map((p) => (p.id === "you" ? { ...p, audioMuted: !enabled } : p))
    );
  };
  ...
  const toggleVideo = () => {
    ...
    setParticipants((prev) =>
      prev.map((p) => (p.id === "you" ? { ...p, videoMuted: !enabled } : p))
    );
  };
    ...
    try {
      ...
      rec.onstop = async () => {
        try {
          ...
          alert(
            "Recording uploaded to Walrus." +
              (result?.id ? ` Blob ID: ${result.id}` : "")
          );
        } catch (err) {
      ...
      <div className="flex-1 relative">
        ...
        <ParticipantsPanel
          ...
          onToggleMute={(id) =>
            setParticipants((prev) =>
              prev.map((p) =>
                p.id === id ? { ...p, audioMuted: !p.audioMuted } : p
              )
            )
          }
          onToggleVideo={(id) =>
            setParticipants((prev) =>
              prev.map((p) =>
                p.id === id ? { ...p, videoMuted: !p.videoMuted } : p
              )
            )
          }
          onRemove={(id) =>
          ...
          onMakeCoHost={(id) =>
            setParticipants((prev) =>
              prev.map((p) => (p.id === id ? { ...p, role: "co-host" } : p))
            )
          }
          onAdmit={(id) =>
            setParticipants((prev) =>
              prev.map((p) => (p.id === id ? { ...p, inLobby: false } : p))
            )
          }
          canManage={role === "host"}
          onAdd={(nameOrAddress) =>
          ...
          <div className="flex items-center gap-3 bg-gray-900/90 text-white px-4 py-3 rounded-full border border-gray-800">
            <button
              ...
              className={`px-3 py-2 rounded ${
                audioEnabled ? "bg-gray-700" : "bg-red-700"
              }`}
            >
            ...
            <button
              onClick={toggleVideo}
              className={`px-3 py-2 rounded ${
                videoEnabled ? "bg-gray-700" : "bg-red-700"
              }`}
            >
            ...
            {role === "host" && (
              <>
                ...
                <button
                  onClick={endRoomOnChain}
                  disabled={chainBusy || !hostCapId}
                  className="px-3 py-2 rounded bg-yellow-700 disabled:opacity-50"
                >
                  End On-Chain
                </button>
              </>
            )}
            <button onClick={endCall} className="px-3 py-2 rounded bg-red-700">
